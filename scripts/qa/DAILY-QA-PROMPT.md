...
8. BLIND-SPOT SWEEP (James's law, 2026-07-16: "what is going on in the world via weather that we're not picking up on — we should pick up on all of it"). Build EXTERNAL ground truth first, read the product second, then diff:
   a. NWS live: `curl -s "https://api.weather.gov/alerts/active?status=actual&severity=Severe,Extreme" -H "User-Agent: dcd-qa (jayhillendalepress@gmail.com)"` — tally event_type × state; name the top 3 stories by scale (multi-state clusters, unusual event types).
   b. Air: today's hunt-air-quality rows (archive lane, embedded daily 06:15) — list every state at AQI ≥ 100; ≥ 150 anywhere is automatically a top story.
   c. Drought: current US Drought Monitor national statistics (usdmdataservices.unl.edu, GetDroughtSeverityStatisticsByAreaPercent, aoi=us, most recent Tuesday) — note D2+ area percent and any state jumping a class. Drought always leaves footprints — look for them.
   d. The product's voice today: porch sentence + coda + alert rings (check-7 shots), the Morning Line, /court.
   e. For each top external story: does ANY surface SPEAK it? Each miss = one ⚠️ BLIND SPOT line: the story, which archive lane already holds it (or "no lane exists"), and which surface should say it. Blind spots are WARN by default (the board's nerves are a build decision, not a regression) — but the SAME blind spot appearing in 3 consecutive daily reports escalates to FAIL. An honestly-labeled gap is still a blind spot; the sweep's job is to keep naming it until it's built or consciously declined.
