...
3. CRON HEALTH: hunt_cron_log rows from the last 26 h for hunt-frame-daily, hunt-board-rhyme, hunt-alert-grader, hunt-claim-court (query by function name + created_at bound is fine on this small table). Any status=error run = FAIL for that function; a missing run for frame/rhyme = FAIL.
4. MORNING LINE: /functions/v1/hunt-morning-line — headline+lede present, parts.anomaly.day0_source='live', no "(2025)" almanac framing. Fetch ?date={yesterday} too: the two ledes must not be verbatim-identical (mad-lib regression check).
