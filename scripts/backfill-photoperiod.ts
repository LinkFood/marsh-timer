/**
 * Backfill photoperiod (day length) embeddings for all 50 states across 5 years.
 * Pure math — no external API needed for solar calculations.
 * Embeds every 3rd day to keep volume reasonable (~30K entries).
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=... VOYAGE_API_KEY=... npx tsx scripts/backfill-photoperiod.ts
 *
 * Resume options:
 *   START_STATE=TX  — skip states before TX alphabetically
 *   START_DATE=2023-06-15 — skip dates before this within the start state
 */
...
// --- State centroids (lat only needed) ---
...
/** Simplified NOAA solar calculator — returns hours */
function calcDayLength(lat: number, doy: number): { sunrise: number; sunset: number; dayLength: number } {
  ...
  if (cosArg > 1) return { sunrise: 12, sunset: 12, dayLength: 0 }; // polar night
  if (cosArg < -1) return { sunrise: 0, sunset: 24, dayLength: 24 }; // midnight sun
...
/** Civil twilight uses -6 degrees instead of -0.8333 */
...
/** Days to nearest equinox or solstice. Returns { event, days } */
function daysToNearestAstronomicalEvent(date: Date): { event: string; days: number } {
  const year = date.getFullYear();
  // Approximate dates for Northern Hemisphere
  const events = [
    { event: "spring_equinox", date: new Date(year, 2, 20) },   // Mar 20
    { event: "summer_solstice", date: new Date(year, 5, 21) },  // Jun 21
    { event: "fall_equinox", date: new Date(year, 8, 22) },     // Sep 22
    { event: "winter_solstice", date: new Date(year, 11, 21) }, // Dec 21
    // Also check next year's spring equinox and prev year's winter solstice
...
function prepareEntry(abbr: string, lat: number, date: Date, prevDayLength: number | null): PreparedEntry {
  ...
  // Daily change in seconds vs previous day
  let changeSeconds = 0;
  let changeSign = "+";
  ...
  // Crossing 12h: day length is near 12 (within 15 min)
...
async function main() {
  ...
  const allDates = generateDates("2021-01-01", "2026-12-31", 3); // every 3rd day
  const totalPerState = allDates.length;
  const totalEntries = totalPerState * STATE_ABBRS.length;

  console.log("=== Backfill Photoperiod Embeddings ===");
  console.log(`States: ${STATE_ABBRS.length} | Dates: ${totalPerState} (every 3rd day) | Total: ${totalEntries}`);
  if (START_STATE) console.log(`Resuming from state: ${START_STATE}`);
  if (START_DATE) console.log(`Resuming from date: ${START_DATE}`);
  ...
  for (const abbr of STATE_ABBRS) {
    ...
    let prevDayLength = calcDayLength(lat, dayOfYear(prevDate)).dayLength;

    // For daily change accuracy with 3-day gaps, compute the day *before* each date
    // instead of using the previous entry's day length.

    let skippingDate = !!(START_STATE && abbr === START_STATE && START_DATE);
    let stateCount = 0;
    let pendingRows: any[] = [];
    ...
    for (let i = 0; i < allDates.length; i++) {
      ...
      const entry = prepareEntry(abbr, lat, date, prevDL);
      ...
      if (batchTexts.length === 20 || i === allDates.length - 1) {
        ...
        const batchEnd = dateStr;
        console.log(`  ${batchStart} to ${batchEnd} (${batchTexts.length} embedded, ${stateCount}/${totalPerState} state, ${globalCount}/${totalEntries} total)`);
