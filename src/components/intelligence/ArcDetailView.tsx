...
import { useArcDetail } from '@/hooks/useArcDetail';
import ArcTimeline from './ArcTimeline';
import ArcConvergence from './ArcConvergence';
import ArcClaimCard from './ArcClaimCard';
import CountdownClock from './CountdownClock';
import FingerprintMatches from './FingerprintMatches';
import {
  X, BookOpen, Layers, Eye, CheckCircle, AlertTriangle,
  XCircle, HelpCircle, Fingerprint, BarChart3,
} from 'lucide-react';
...
const GRADE_STYLES: Record<string, { bg: string; text: string; icon: typeof CheckCircle; label: string }> = {
  confirmed: { bg: 'bg-emerald-400/20', text: 'text-emerald-400', icon: CheckCircle, label: 'CONFIRMED' },
  partially_confirmed: { bg: 'bg-amber-400/20', text: 'text-amber-400', icon: AlertTriangle, label: 'PARTIAL' },
  missed: { bg: 'bg-red-400/20', text: 'text-red-400', icon: XCircle, label: 'MISSED' },
  false_alarm: { bg: 'bg-gray-500/20', text: 'text-gray-400', icon: HelpCircle, label: 'FALSE ALARM' },
};
...
export default function ArcDetailView({ arc, onClose }: ArcDetailViewProps) {
  const { convergence, loading } = useArcDetail(arc.id);
  ...
  const scoreTrend = (buildup as Record<string, unknown>).score_trend as string || '';
    ...
    <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800 bg-gray-900/95">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <span className="text-lg font-mono font-bold text-white">{arc.state_abbr}</span>
          <span className="text-xs font-mono text-white/40 truncate hidden sm:inline">{stateName}</span>
          {arc.precedent_accuracy != null && (
            <span className={`text-sm font-mono font-bold px-2 py-0.5 rounded ${scoreTierColor(arc.precedent_accuracy)}`}>
              {Math.round(arc.precedent_accuracy)}
            </span>
          )}
          <span className={`text-[9px] font-mono uppercase px-2 py-0.5 rounded shrink-0 ${actColor}`}>
            {arc.current_act}
          </span>
        </div>
        <button
        ...
        <div className="p-4 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1 text-[9px] font-mono text-white/20">
              <span>Opened: {new Date(arc.opened_at).toLocaleDateString()}</span>
              <span>Act started: {new Date(arc.act_started_at).toLocaleDateString()}</span>
              {arc.outcome_deadline && (
                <span className="flex items-center gap-1">
                  Deadline: <CountdownClock deadline={arc.outcome_deadline} />
                </span>
              )}
            </div>
          </div>
            ...
            {arc.narrative ? (
              ...
              <div className="bg-gray-950/50 rounded-lg p-4 border border-gray-800/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono text-white/30">Narrator hasn't processed this arc yet.</span>
                </div>
          ...
          {/* ── D) Convergence Breakdown ── */}
          <div>
            <div className="flex items-center gap-2 mb-2">
              <BarChart3 size={12} className="text-cyan-400" />
              <h4 className="text-[10px] font-mono uppercase tracking-widest text-white/50">Convergence Breakdown</h4>
            </div>
            <div className="bg-gray-950/50 rounded-lg p-4 border border-gray-800/50">
              <ArcConvergence scores={convergence} />
            </div>
          </div>
            ...
            <div>
              ...
              <div className="bg-gray-950/50 rounded-lg p-4 border border-gray-800/50">
                ...
                {convergenceScore != null && (
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[9px] font-mono text-white/40 uppercase">Score at buildup</span>
                    <span className="text-xs font-mono font-bold text-cyan-400">{convergenceScore}</span>
                    {scoreTrend && (
                      <span className="text-[9px] font-mono text-white/30">{scoreTrend}</span>
                    )}
                  </div>
                )}
                {triggerText && (
          ...
          {/* ── F) Recognition Claim ── */}
          {(arc.current_act === 'recognition' || arc.current_act === 'outcome' || arc.current_act === 'grade') && (
            <div>
              <ArcClaimCard claim={arc.recognition_claim || {}} deadline={arc.outcome_deadline} />
            </div>
          )}

          {/* ── G) Outcome Signals ── */}
          {(arc.current_act === 'outcome' || arc.current_act === 'grade') && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Eye size={12} className="text-cyan-400" />
                <h4 className="text-[10px] font-mono uppercase tracking-widest text-white/50">Outcome Signals</h4>
              </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-start gap-3">
                      <div className={`w-16 h-16 rounded-lg flex items-center justify-center shrink-0 ${style.bg}`}>
                        <Icon size={28} className={style.text} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className={`text-lg font-mono font-bold uppercase ${style.text}`}>
                          {style.label}
                        </span>
                        {arc.grade_reasoning && (
                          <div className="mt-2">
                            {renderNarrative(arc.grade_reasoning)}
                          </div>
                        )}
                      </div>
                    </div>
