...
export default function LayerPicker() {
  ...
  return (
    <div
      ref={panelRef}
      className={`fixed top-12 bottom-11 right-0 z-40 ${isMobile ? 'left-0' : 'w-[280px]'} glass-panel border-l border-white/[0.06] flex flex-col overflow-hidden transition-transform duration-300 ease-out ${
        layerPickerOpen ? 'translate-x-0' : 'translate-x-full'
      }`}
    >
      {/* Header */}
      <div className="shrink-0 h-10 px-3 flex items-center justify-between border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-2">
          <button onClick={resetLayers} className="text-white/40 hover:text-white/80 transition-colors" title="Reset to defaults">
            <RotateCcw size={12} />
          </button>
          <button onClick={() => setLayerPickerOpen(false)} className="text-white/40 hover:text-white/80 transition-colors">
            <X size={14} />
          </button>
        </div>
      </div>
                ...
                <div className="pb-1">
                  {layers.map(layer => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-[11px] ${on ? 'text-white/90' : 'text-white/40'} transition-colors`}>{layer.label}</span>
                        <div className={`w-7 h-4 rounded-full relative transition-colors ${on ? 'bg-cyan-400/60' : 'bg-white/[0.08]'}`}>
                          <div className={`absolute top-0.5 w-3 h-3 rounded-full transition-all ${on ? 'left-3.5 bg-cyan-400' : 'left-0.5 bg-white/30'}`} />
                        </div>
                      </button>
                    );
                  })}
